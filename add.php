<?php
    ...
    if (empty($errors)){
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $new_lot = [$_POST['title'], $_POST['first_price'], $_POST['category_id'], $_POST['description'], $_POST['bet_step'], $_POST['date_delection'], $_SESSION['user_id'], $file_path_db];
            $lot['url'] = $file_path;
            $sql = 'INSERT INTO lots (name, first_price, category_id, description, bet_step, date_delection, date_creation, author, url) VALUES (?, ?, ?, ?, ?, ?, NOW(), ?, ?)';
